...
void* ftcs_solver ( void* arguments );
void* external_heat ( void* arguments );
...
void* ftcs_solver( void* arguments ){
   int *these_arguments = (int *) arguments;
    int mystep = these_arguments[0];
    int myrank = these_arguments[1];

    int xIterateLength = GRID_SIZE[0]/n_threads;
    int xStart = xIterateLength*myrank;
    int xMax = xStart+xIterateLength;
    if(myrank == n_threads){
        xMax = GRID_SIZE[0];
    }
    ...
    for(int x = xStart; x < xMax; x++){
        for(int y = 0; y < GRID_SIZE[1]; y++){
            float* in = temperature[(mystep)%2];
            float* out = temperature[(mystep+1)%2];
...
void* external_heat( void* arguments ){
    ...
    int xIterateLength = ((3*GRID_SIZE[0]/4) - (GRID_SIZE[0]/4))/n_threads;
    int xStart = (GRID_SIZE[0]/4) + myrank*xIterateLength;
    int xMax = xStart + xIterateLength;
    if(myrank == n_threads){
        xMax = GRID_SIZE[0];
    }
    if(mystep % 500 == 0){
        //printf("xStart: %d, xMax: %d, rank: %d\n", xStart, xMax, myrank);        
    }

    //int yIterateLength = (((GRID_SIZE[1]/2)+(GRID_SIZE[1]/16)) - ((GRID_SIZE[1]/2)-(GRID_SIZE[1]/16))) / n_threads;
    int yStart = (GRID_SIZE[1]/2)-(GRID_SIZE[1]/16);
    int yMax = (GRID_SIZE[1]/2)+(GRID_SIZE[1]/16);
/*
    int yIterateLength = (((GRID_SIZE[1]/2)+(GRID_SIZE[1]/16)) - ((GRID_SIZE[1]/2)-(GRID_SIZE[1]/16))) / n_threads;
    int yStart = ((GRID_SIZE[1]/2)-(GRID_SIZE[1]/16)) + rank*yIterateLength;
    int yMax = yStart + yIterateLength;
*/
    for(int x = xStart; x < xMax; x++){
        for(int y = yStart; y < yMax; y++){
            temperature[mystep%2][ti(x, y)] = 100.0;
        }
    }
    return NULL;
}
...
int main ( int argc, char **argv ){
    ...
    for( int step=0; step<NSTEPS; step += 1 ){
        if( step < CUTOFF ){
            int *arguments = (int*)malloc(n_threads*2*sizeof(int));
            for(int thread = 0; thread<n_threads; thread++){
                arguments[0+(2*thread)] = step;
                arguments[1+(2*thread)] = thread;
                pthread_create(&thread_handles[thread], NULL, external_heat, ((void*) arguments + thread*2*sizeof(int)));

                // if(step % 500 == 0){
                //     printf("thread created, heat: %d, step: %d\n", thread, step );
                // }
            }
            free(arguments);
        }
        ...
        for(int thread = 0; thread < n_threads; thread++){
            ...
            // if(step % 500 == 0){
            //         printf("thread stopped, heat: %d, step: %d\n", thread, step );
            // }
        }
        ...
        for(int thread = 0; thread<n_threads; thread++){
            arguments[0+(2*thread)] = step;
            arguments[1+(2*thread)] = thread;
            pthread_create(&thread_handles[thread], NULL, ftcs_solver, ((void*) arguments + thread*2*sizeof(int)));

            // if(step % 500 == 0){
            //     printf("thread created, solver: %d, step: %d\n", thread, step );
            // }
        }
        free(arguments);
        ...
        for(int thread = 0; thread < n_threads; thread++){
            pthread_join(thread_handles[thread], NULL);
            // if(step % 500 == 0){
            //         printf("thread stopped, solver: %d, step: %d\n", thread, step );
            // }
        }
